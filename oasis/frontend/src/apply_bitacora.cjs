...
// 4. Wrapping Header and Content with {viewMode === 'canvases' ? (...) : (...)}
...
// The closing tag!
// The last lines of BitacoraExistencial are:
//             </div>
//         </div>
//     );
// }
// We need to inject `</> )}` right before the second to last `</div>`.
const closingRegex = /            <\/div>\n        <\/div>\n    \);\n\}/;
const closingInjection = `            </div>\n                </>\n            )}\n        </div>\n    );\n}`;
content = content.replace(closingRegex, closingInjection);


// 5. Replace the Header to add the ViewMode button
const headerRegex = /<div className=\"flex items-center gap-2\">\s*<Eye size=\{18\} className=\"text-red-500 animate-pulse\" \/>\s*<span className=\"text-xs font-black uppercase tracking-\[0\.15em\] text-white\">BitÃ¡cora Existencial<\/span>\s*<\/div>/g;
...
console.log('Bitacora full injection completed cleanly.');
