...
import { ArrowLeft, Search, Trash2, Check, X, Columns } from 'lucide-react';
...
const SimpleNotesView = React.forwardRef(({ blocks, setBlocks, accent, onClose, user, editBlock, openNewComposer, className, isSplitView, onToggleSplitView }, ref) => {
    ...
    const notes = blocks
        ...
        .sort((a, b) => {
            let tA = getBlockTime(a);
            let tB = getBlockTime(b);
            if (isNaN(tA)) tA = 0;
            if (isNaN(tB)) tB = 0;
            return tB - tA;
        });
                ...
                <div className="flex items-center justify-center mb-4 relative h-9">
                    {onToggleSplitView && (
                        <button
                            ...
                            className={`absolute left-0 top-1/2 -translate-y-1/2 p-2 rounded-xl transition-all hidden md:flex ${isSplitView ? 'bg-accent/20 text-accent' : 'text-zinc-500 hover:text-white hover:bg-white/5'}`}
                    ...
                    <button
                        ...
                        className="absolute right-0 top-1/2 -translate-y-1/2 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest text-black transition-all hover:scale-105 active:scale-95 shadow-[0_0_10px_rgba(var(--accent-rgb),0.3)]"
