...
export const DiaryNotebook = ({ activeCanvasId, onClose, onFocusNode, blocks, setBlocks, syncBlocks, accent, className }) => {
    ...
    const diaryBlocks = blocks.filter(b => {
        if (!b.entries || b.entries.length === 0) return false;
    ...
    const handleSave = () => {
        ...
        const existingBlock = currentBlocks.find(b => b.id === entryId || (activeEntryId && b.id === activeEntryId));
        ...
        const exists = currentBlocks.some(b => b.id === entryId || (activeEntryId && b.id === activeEntryId));
        const updated = exists 
            ? currentBlocks.map(b => (b.id === entryId || (activeEntryId && b.id === activeEntryId)) ? newBlock : b)
                ...
                <div className="relative z-10 flex-1 overflow-y-auto no-scrollbar pt-12 md:pt-16 min-w-0">
                    <div className="max-w-2xl mx-auto px-8 md:px-12 space-y-6 pb-32">
                        ...
                        <textarea 
                            ...
                            className="w-full bg-transparent border-none text-3xl md:text-4xl font-bold tracking-tight text-white/90 placeholder:text-zinc-800 focus:outline-none focus:ring-0 px-0 font-sans resize-none overflow-hidden"
                        />
