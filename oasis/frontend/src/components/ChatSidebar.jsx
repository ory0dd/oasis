...
const ChatSidebar = ({
    ...
    return (
        <div className="fixed md:relative inset-y-0 left-0 w-[280px] sm:w-80 h-full bg-[#080809] border-r border-white/5 flex flex-col animate-in slide-in-from-left duration-300 z-[1750] shadow-2xl md:shadow-none transition-colors duration-1000">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar px-6 pb-6 space-y-8">
                {activeTab === 'chats' ? (
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-6">
                        <div className="space-y-4">
                            <div className="space-y-2">
                                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500/80 flex items-center gap-2"><Zap size={12} /> Red de Contexto</h3>
                                <p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Conocimiento activo de Kio sobre ti.</p>
                            </div>
                            
                            {(() => {
                                let afcNodes = [];
                                try {
                                    const data = JSON.parse(localStorage.getItem(`oasis_afc_real_data_${user}`));
                                    if (data && data.nodes) afcNodes = data.nodes;
                                } catch(e) {}
                                
                                const recentConvs = conversations.filter(c => c.id !== activeConversationId).sort((a, b) => new Date(b.startTime || 0) - new Date(a.startTime || 0)).slice(0, 5);
                                
                                return (
                                    <div className="space-y-2">
                                        <div className="grid grid-cols-2 gap-2">
                                            <div 
                                                onClick={() => setExpandedContext(expandedContext === 'afc' ? null : 'afc')}
                                                className={`p-3 rounded-2xl flex flex-col gap-1 cursor-pointer transition-all border ${expandedContext === 'afc' ? 'bg-emerald-500/20 border-emerald-500/40' : 'bg-emerald-500/10 border-emerald-500/20 hover:bg-emerald-500/20'}`}
                                            >
                                                <span className="text-[7px] font-black uppercase tracking-widest text-emerald-500">Mapa Conductual</span>
                                                <span className="text-[9px] font-bold text-white/80">{afcNodes.length > 0 ? `${afcNodes.length} Nodos activos` : 'Inactivo'}</span>
                                            </div>
                                            <div 
                                                onClick={() => setExpandedContext(expandedContext === 'conversations' ? null : 'conversations')}
                                                className={`p-3 rounded-2xl flex flex-col gap-1 cursor-pointer transition-all border ${expandedContext === 'conversations' ? 'bg-sky-500/20 border-sky-500/40' : 'bg-sky-500/10 border-sky-500/20 hover:bg-sky-500/20'}`}
                                            >
                                                <span className="text-[7px] font-black uppercase tracking-widest text-sky-500">Chats Previos</span>
                                                <span className="text-[9px] font-bold text-white/80">{recentConvs.length} Conversaciones</span>
                                            </div>
                                        </div>
                                        
                                        {expandedContext === 'afc' && afcNodes.length > 0 && (
                                            <div className="max-h-48 overflow-y-auto no-scrollbar space-y-1.5 bg-black/20 p-2 rounded-xl border border-white/5 animate-in slide-in-from-top-2">
                                                {afcNodes.map((n, i) => (
                                                    <div key={i} className="p-2.5 bg-emerald-950/20 rounded-lg border border-emerald-900/30">
                                                        <p className="text-[9px] font-bold text-emerald-400">{n.label || 'Nodo'}</p>
                                                        <p className="text-[8px] text-white/50 mt-1 line-clamp-2 leading-relaxed">{n.description || 'Sin descripción'}</p>
                                                        {n.intensity && <p className="text-[7px] text-emerald-500/60 mt-1 uppercase font-black">Intensidad: {n.intensity}</p>}
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                        
                                        {expandedContext === 'conversations' && recentConvs.length > 0 && (
                                            <div className="max-h-48 overflow-y-auto no-scrollbar space-y-1.5 bg-black/20 p-2 rounded-xl border border-white/5 animate-in slide-in-from-top-2">
                                                {recentConvs.map((c, i) => (
                                                    <div key={i} className="p-2.5 bg-sky-950/20 rounded-lg border border-sky-900/30">
                                                        <p className="text-[9px] font-bold text-sky-400 uppercase tracking-wider">{c.title || 'Sin Título'}</p>
                                                        <p className="text-[8px] text-white/60 mt-1 line-clamp-2 leading-relaxed italic">Última act: {c.startTime ? new Date(c.startTime).toLocaleDateString() : 'Desconocida'}</p>
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                        ...
                        <div className="space-y-2 pt-4 border-t border-white/5"><h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-accent/60 flex items-center gap-2"><Zap size={12} /> Núcleo de Memoria</h3><p className="text-[9px] font-bold text-white/30 uppercase tracking-widest">Hechos destilados de tus chats.</p></div>
            ...
            <div className="p-6 pb-20 border-t border-white/[0.02] bg-white/[0.01] space-y-6">
                <div className="flex items-center justify-between">
