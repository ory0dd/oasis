...
if (content.includes(navBarStart) && content.includes('title="Lienzo Principal"')) {
    const startIndex = content.indexOf(navBarStart);
    const endIndex = content.indexOf(')}', content.indexOf('title="Lienzo Principal"', startIndex)) + 2;
...
if (!content.includes('<BitacoraExistencial')) {
    const backdropStart = `{/* UNIFIED MODAL BACKDROP FOR MOBILE */}`;
    const newModals = `
            {isBitacoraOpen && (
                <BitacoraExistencial
                    blocks={blocks}
    ...
    // The old backdrop might be multiline. It's safer to replace the start and the following lines.
