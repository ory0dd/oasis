...
// 1. Añadir props activeCanvasId y setActiveCanvasId
content = content.replace(
    /export default function BitacoraExistencial\(\{\s*blocks, setBlocks,/g,
    `export default function BitacoraExistencial({ \n    activeCanvasId, setActiveCanvasId,\n    blocks, setBlocks,`
);

// 2. Añadir estado viewMode, states para renombrar
...
content = content.replace(/const filteredReleases = blocks\.filter\(b => \{\s*if \(b\.id === 'profile_settings'/g, filteredReleasesInjection);


// 4. Inyectar vista de Canvases si viewMode === 'canvases'
// We will wrap everything inside the <div onWheel={...}> with {viewMode === 'canvases' ? (...) : (<> ... </>)}
...
const topBarRegex = /\{\/\* TOP BAR \/ HEADER \*\/\}/;
if (!content.includes('viewMode === \'canvases\'')) {
    content = content.replace(topBarRegex, canvasViewHTML + '\n            {/* TOP BAR / HEADER */}');
    
    // Add the closing tag for the fragment right before the final </div>
    const finalDivRegex = /(<\/div>\s*)\s*(\);\s*\})/;
    content = content.replace(finalDivRegex, '                </>\n            )\}\n$1$2');
}

fs.writeFileSync(file, content);
console.log('Fully applied Bitacora injection.');
