...
const startBucles = content.indexOf(`ISLAS EXISTENCIALES (PATRONES CONDUCTUALES)`);
const endBucles = content.indexOf(`RASGOS PID-5 E INTEGR`);
const actualStart = content.lastIndexOf(`{/* `, startBucles);
const actualEnd = content.lastIndexOf(`{/* `, endBucles);

let block = content.substring(actualStart, actualEnd);

const match = block.match(/\{\/\* Content body \*\/\}([\s\S]*?)Explorar con Kio IA[\s\S]*?<\/button>\s*<\/div>\s*<\/div>/);
if (!match) {
    console.error("Regex match failed");
    process.exit(1);
}

let extractedDetails = match[0];
// Remove the trailing wrapper divs that we matched (from the original pattern-details-panel footer)
extractedDetails = extractedDetails.substring(0, extractedDetails.lastIndexOf('</button>') + 9);
extractedDetails += `\n                                        </div>`; // add closing div for the footer actions container

// Replace activePattern with pat
extractedDetails = extractedDetails.replace(/activePattern/g, 'pat');

// Remove internal scrollbar from Content body since it's now an inline accordion
extractedDetails = extractedDetails.replace(/overflow-y-auto max-h-\[320px\] sm:max-h-\[360px\] custom-scroll pr-1\.5/g, '');

const newBuclesPage = `                            {/* BUCLES PAGE (LIST FORMAT) */}
                            {mapViewTab === 'bucles' && (
...
const newContent = content.substring(0, actualStart) + newBuclesPage + content.substring(actualEnd);
