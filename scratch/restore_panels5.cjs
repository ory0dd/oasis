...
// In case the newlines were not matching
if (content.includes(`{isEmbedded && (`)) {
    // wait, we only want to replace the FIRST remaining one, because the first one was already replaced (hopefully)
    let idx = content.indexOf(`{isEmbedded && (`);
    if (idx !== -1) {
        let replacement = `{mapViewTab === 'exit_keys' && (`;
        content = content.substring(0, idx) + replacement + content.substring(idx + 17);
    }
    content = content.replace(
        `w-[500px] max-w-[calc(100vw-3rem)] z-[200]`,
        `w-[400px] max-w-[calc(100vw-3rem)] z-[200]`
    );
}
