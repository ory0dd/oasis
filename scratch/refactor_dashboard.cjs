...
// 1. Add state variable for active floating panel
...
code = code.replace(
    ...
    '<div className="absolute inset-0 flex-1 w-full h-full animate-in slide-in-from-bottom-8 duration-700 delay-200 fill-mode-both">'
);
...
// Also add top padding to the header inside MÓDULO 1 to account for the lack of pt-[96px] in the parent
code = code.replace(
...
code = code.replace(
    ...
    '<h2 className="text-sm font-bold text-white flex items-center gap-1.5 glassmorphism px-4 py-2 rounded-xl bg-black/40 border border-white/10 backdrop-blur-md">'
);

// Close the injected div structure correctly around the action buttons
code = code.replace(
    '<div className="flex flex-wrap items-center gap-4">',
    '</div>\n<div className="flex flex-wrap items-center gap-4 pointer-events-auto glassmorphism p-1.5 rounded-xl bg-black/40 border border-white/10 backdrop-blur-md">'
);


// 6. Dock and Panels wrapper
// We need to inject the Dock at the bottom of MÓDULO 1, and wrap MÓDULO 1.5, 2, 3, 4 into slide-out panels.
// Find the end of MÓDULO 1 (right before MÓDULO 1.5)
...
// Replace MÓDULO 1.5 container
code = code.replace(
    '<div className="bg-zinc-950/60 backdrop-blur-xl border border-white/5 rounded-[2rem] p-4 md:p-6 shadow-xl">',
    '{activeFloatingPanel === \'islas\' && (\n<div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 w-[95%] max-w-6xl z-40 bg-zinc-950/80 backdrop-blur-2xl border border-white/10 rounded-[2rem] p-4 md:p-6 shadow-2xl animate-in slide-in-from-bottom-8">'
);
...
code = code.replace(
    ...
    '{activeFloatingPanel === \'tratamiento\' && (\n<div className="absolute top-0 right-0 h-full w-[500px] max-w-full z-40 bg-zinc-950/90 backdrop-blur-3xl border-l border-white/10 p-6 overflow-y-auto animate-in slide-in-from-right-8 shadow-2xl">\n<div className="flex justify-between items-center mb-6"><h3 className="text-lg font-black text-white">Plan de Tratamiento</h3><button onClick={() => setActiveFloatingPanel(null)} className="p-2 bg-white/5 rounded-full hover:bg-white/10 text-white"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button></div>\n{/* MÓDULO 4: PLAN DE TRATAMIENTO */}'
);

// Close MÓDULO 4 conditional render at the end of the viewMode block
const endOfViewModeBlock = code.lastIndexOf('</div>\n                )}\n            </div>\n        </div>');
code = code.slice(0, endOfViewModeBlock) + '</div>\n)}\n' + code.slice(endOfViewModeBlock);
